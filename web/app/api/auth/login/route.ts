    ...
    try {
        ...
        if (username === 'admin' && password === 'admin123') {
            ...
            if (user) {
                ...
                await createSession(user.id.toString(), user.companyId || 1, user.role || 'SUPERADMIN', user.name, user.email, user.phone);

                // Log Activity
                try {
                    db.prepare('INSERT INTO ActivityLog (userId, action, description, companyId) VALUES (?, ?, ?, ?)')
                        .run(user.id, 'LOGIN', 'Admin masuk via kredensial magic', user.companyId || 1);
                } catch (e) { }
        ...
        await createSession(user.id.toString(), user.companyId, user.role, user.name, user.email, user.phone);

        // Log Activity
        try {
            db.prepare('INSERT INTO ActivityLog (userId, action, description, companyId) VALUES (?, ?, ?, ?)')
                .run(user.id, 'LOGIN', 'Pengguna berhasil masuk', user.companyId);
        } catch (e) { }
