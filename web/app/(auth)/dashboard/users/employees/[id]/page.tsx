        ...
        <div className="p-6 max-w-5xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-slate-800">{employee.user.name}</h1>
                        <p className="text-slate-500">{employee.position} &bull; {employee.department}</p>
                    </div>
                </div>
                <Button asChild>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <Card className="md:col-span-1 shadow-sm">
                    <CardContent className="pt-6 flex flex-col items-center text-center">
                        ...
                        <Badge variant="outline" className="mb-2 uppercase tracking-wide text-[10px]">{employee.department}</Badge>
                        <div className="text-sm text-slate-500">{employee.user.email}</div>
                    </CardContent>
                </Card>
                ...
                <div className="md:col-span-3">
                    <Tabs defaultValue="profile">
                        <TabsList className="grid w-full grid-cols-3">
                            <TabsTrigger value="profile">{t.userDetail.tabs.profileEmployee}</TabsTrigger>
                        ...
                        <TabsContent value="profile">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-slate-800">{t.userDetail.profile.titleEmployee}</CardTitle>
                                    <CardDescription>{t.userDetail.profile.subtitleEmployee}</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <Label className="text-xs text-slate-500 uppercase">{t.userDetail.profile.position}</Label>
                                            <div className="flex items-center gap-2 font-medium">
                                                <Briefcase className="h-4 w-4 text-slate-400" /> {employee.position}
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <Label className="text-xs text-slate-500 uppercase">{t.userDetail.profile.department}</Label>
                                            <div className="flex items-center gap-2 font-medium">
                                                <Building2 className="h-4 w-4 text-slate-400" /> {employee.department}
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <Label className="text-xs text-slate-500 uppercase">{t.userDetail.profile.baseSalary}</Label>
                                            <div className="font-medium">Rp {employee.baseSalary.toLocaleString('id-ID')}</div>
                                        </div>
                                        <div className="space-y-1">
                                            <Label className="text-xs text-slate-500 uppercase">{t.userDetail.profile.joinDate}</Label>
                        ...
                        <TabsContent value="documents">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-slate-800">{t.userDetail.documents.title}</CardTitle>
                                    <CardDescription>{t.userDetail.documents.subtitle}</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-6">

                                    {/* Upload Form */}
                                    <div className="p-4 bg-slate-50 border border-dashed border-slate-200 rounded-lg">
                                        <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Upload className="h-4 w-4" /> {t.userDetail.documents.uploadTitle}</h3>
                                    ...
                                    <div className="space-y-2">
                                        {documents.map((doc: any) => (
                                            <div key={doc.id} className="flex items-center justify-between p-3 border rounded-md hover:bg-slate-50 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500">{doc.type} &bull; {new Date(doc.uploadedAt).toLocaleDateString()}</div>
                                                    </div>
                                                </div>
                                                <div className="flex gap-2">
                                                    <Button variant="ghost" size="sm" asChild>
                                                        <Link href={`/preview/document/${doc.id}`} target="_blank">
                            ...
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <div className="flex flex-col space-y-1.5">
                                        ...
                                        <CardDescription>{t.userDetail.activity.subtitleEmployee}</CardDescription>
                                    </div>
